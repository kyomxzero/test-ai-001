...
class HolidayCalendarHelper {
  ...
  async searchHoliday(criteria: { field: string; operator: string; value: string }) {
    try {
      ...
      // Select field from dropdown (like Holidays Year, Created By, etc.)
      const fieldDropdown = this.page.locator('select, .ant-select, [role="combobox"]').first();
      if (await fieldDropdown.count() > 0) {
        await fieldDropdown.click();
        await this.page.waitForTimeout(500);
        
        // Look for the field option
        const fieldOption = this.page.locator(`text="${criteria.field}", [title="${criteria.field}"]`);
        if (await fieldOption.count() > 0) {
          await fieldOption.click();
        } else {
          // Try typing the field name
          await this.page.keyboard.type(criteria.field);
  ...
  async searchByHolidayYear(year: string) {
    try {
      // Use the general search method for Holiday Year
      await this.searchHoliday({ field: 'Holidays Year', operator: '=', value: year });
    } catch (error) {
      console.log('Error searching by holiday year:', error);
    }
  }
...
test.describe('Holiday Calendar Master Tests - All 66 Test Cases', () => {
  ...
  test('TC-02-003: Search Holiday by Year equals operator', async ({ page }) => {
    await test.step('Navigate to Holiday Calendar', async () => {
      await helper.navigateToHolidayCalendar();
      await helper.takeScreenshot('screenshots/TC-02-003/01-initial_page.png');
    });

    await test.step('Search by Holiday Year equals 2026', async () => {
      await helper.searchHoliday({ field: 'Holidays Year', operator: '=', value: '2026' });
      await helper.takeScreenshot('screenshots/TC-02-003/02-search_results.png');
    });
  ...
  test('TC-02-004: Search Holiday by Year not equals operator', async ({ page }) => {
    await test.step('Navigate to Holiday Calendar', async () => {
      await helper.navigateToHolidayCalendar();
      await helper.takeScreenshot('screenshots/TC-02-004/01-initial_page.png');
    });

    await test.step('Search by Holiday Year not equals 2025', async () => {
      await helper.searchHoliday({ field: 'Holidays Year', operator: '<>', value: '2025' });
      await helper.takeScreenshot('screenshots/TC-02-004/02-search_results.png');
    });

    await test.step('Export search results', async () => {
      await helper.clickButton('Export');
      await page.waitForTimeout(2000);
      await helper.takeScreenshot('screenshots/TC-02-004/03-export_results.png');
    });
  });
  ...
  test('TC-02-005: Search Holiday by Year greater than operator', async ({ page }) => {
    await test.step('Navigate to Holiday Calendar', async () => {
      await helper.navigateToHolidayCalendar();
      await helper.takeScreenshot('screenshots/TC-02-005/01-initial_page.png');
    });
