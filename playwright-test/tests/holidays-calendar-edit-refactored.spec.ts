...
test.describe('Holiday Calendar Edit Tests', () => {
   ...
   */
  test('TC-02-048_0001: Edit Holiday with complete data and cancel save', async ({ page }) => {
  ...
  /**
   * TC-02-052: Edit Holiday with data entry, clear data, and save
   */
  test('TC-02-052_0001: Edit Holiday with data entry, clear data, and save', async ({ page }) => {
    ...
    try {
      ...
      await test.step('Store original form data', async () => {
        const originalData = await holidayEditPage.getFormValues();
        await takeTestStepScreenshot(page, testCase, '08', 'original_data_captured');
        console.log('✓ Original form data captured:', originalData);
      });

      // Fill holiday information
      await test.step('3. กรอกข้อมูลวันหยุด', async () => {
        const holidayData = HolidayEditPage.getTestData('CLEAR_TEST');
        await holidayEditPage.fillHolidayForm(holidayData, testCase);
      });

      // Click Clear button
      await test.step('4. กดปุ่ม Clear', async () => {
        await holidayEditPage.clickClearButton(testCase);
      });

      // Validate Clear functionality
      await test.step('Validate Clear: ตรวจสอบการเคลียร์ข้อมูลและแสดงค่าเดิม', async () => {
        const isCleared = await holidayEditPage.verifyFormCleared();

        if (isCleared) {
          console.log('✓ EXPECTED RESULT 1 PASSED: Clear function worked');
        } else {
          console.log('⚠ EXPECTED RESULT 1 WARNING: Form may not be completely cleared');
        }

        await takeTestStepScreenshot(page, testCase, '13', 'clear_validation');
      });

      // Save and validate confirmation
      await test.step('5. Save and validate confirmation', async () => {
        await holidayEditPage.saveWithConfirmation(testCase);

        // Check for success message
        await page.waitForTimeout(TIMEOUTS.LONG);
        await takeTestStepScreenshot(page, testCase, '17', 'success_popup');
        console.log('✓ Save operation completed');
      });

      // Final validation
      await test.step('Final Validation Summary', async () => {
        await takeTestStepScreenshot(page, testCase, '20', 'final_state');
